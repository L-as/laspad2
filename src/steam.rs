...
impl<'a> ItemUpdater<'a> {
	...
	pub fn change_description(&self, change_description: &str) -> Result<&Self, ()> {
		if unsafe {
			SteamAPI_ISteamRemoteStorage_UpdatePublishedFileSetChangeDescription(
				self.storage.0,
				self.handle,
				CString::new(change_description).unwrap().as_ptr(),
			)
		} {
			Ok(&self)
		} else {
	...
	pub fn commit(&self) -> APICall {
		unsafe {
			SteamAPI_ISteamRemoteStorage_CommitPublishedFileUpdate(self.storage.0, self.handle)
		}
	}
}
...
impl RemoteStorage {
	...
	pub fn publish_workshop_file(
		...
		let tags = Strings {
			elements: tags
				.iter()
				.map(|&s| CString::new(s).unwrap().as_ptr())
				.collect::<Vec<_>>()
				.as_ptr(),
			length:   tags.len() as i32,
		};
	...
	pub fn update_workshop_file<'a>(&'a mut self, item: Item) -> ItemUpdater<'a> {
		ItemUpdater {
			handle:  unsafe {
				SteamAPI_ISteamRemoteStorage_CreatePublishedFileUpdateRequest(self.0, item)
			},
			storage: self,
		}
	}
...
impl Client {
	pub fn new() -> Result<Self, Error> {
		...
		let client: *mut ClientImpl = unsafe {
			transmute(SteamInternal_CreateInterface(
				CString::new("SteamClient017").unwrap().as_ptr(),
			))
		};
	...
	pub fn remote_storage(&self) -> Result<RemoteStorage, Error> {
		...
		let storage = unsafe {
			SteamAPI_ISteamClient_GetISteamRemoteStorage(
				...
				user,
				pipe,
				CString::new("STEAMREMOTESTORAGE_INTERFACE_VERSION014")
					.unwrap()
					.as_ptr(),
			)
		};
			...
			!storage.is_null(),
			"Could not retrive steam's remote storage API"
		);
	...
	pub fn utils(&self) -> Result<Utils, Error> {
		...
		let utils = unsafe {
			SteamAPI_ISteamClient_GetISteamUtils(
				self.0,
				pipe,
				CString::new("SteamUtils009").unwrap().as_ptr(),
			)
		};
